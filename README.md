...
A single-file, offline, client-side **TOTP (Time-based One-Time Password)** generator. Enter or generate a Base32 TOTP secret, and the page displays a live 6-digit code that automatically refreshes every 30 seconds — the same mechanism used by apps like Google Authenticator or Microsoft Authenticator.

Everything runs **entirely in your browser**. There is no backend, no build step, no external dependency, and no network call of any kind — the TOTP secret never leaves the device it was entered on.
...
- **Generate or enter a TOTP secret** — create a fresh, cryptographically random Base32 secret with one click, or paste in an existing one.
...
- **Optional local persistence** — an explicit "remember this secret on this device" checkbox stores the secret in the browser's `localStorage` so it survives a page reload; it is never saved unless this box is checked.
...
1. The TOTP secret you provide (or that is generated for you) — RFC 6238's "shared secret," conventionally denoted `K` — is encoded in **Base32** ([RFC 4648](https://datatracker.ietf.org/doc/html/rfc4648)), the same encoding used by every major authenticator app.
...
3. That counter is signed with **HMAC** (SHA-1, SHA-256, or SHA-512) using the TOTP secret as the HMAC key, via the browser's native [Web Crypto API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Crypto_API).
...
Because this is a deterministic function of *(secret, time, algorithm, digits, period)*, any other TOTP-compliant application configured with the exact same TOTP secret and the same settings (SHA-1, 6 digits, 30-second period being the near-universal defaults) will always compute the **same code at the same moment** — that's what makes it interoperable with standard authenticator apps.
...
1. **Get a TOTP secret.**
   - Click **"Generate random secret"** to create a new, cryptographically random 160-bit Base32 secret, or
   - paste an existing Base32 secret into the **TOTP secret** field (spaces are ignored, so values copied in groups of 4 characters work fine).
2. **Read the live code.** As soon as a valid secret is present, the OTP panel appears: a large 6-digit code, a countdown ring, and a label showing the active algorithm/digits/period combination.
3. **Copy the code** with the **Copy** button next to it.
4. **Reveal or hide the secret** with the eye icon next to the input field (the field is masked, like a password field, by default).
5. **Remember the secret on this device** (optional) by checking the box underneath the secret actions. This stores the secret in `localStorage`, scoped to this page's origin, so it's pre-filled the next time you open the page **on the same browser and device**. Leave it unchecked to keep the secret in memory only, cleared as soon as the page is closed or reloaded.
6. **Clear everything** with the **Clear** button — it empties the field, unchecks "remember", and removes the secret from `localStorage`.
...
Changing any of these settings changes the generated codes — the same TOTP secret with different settings produces a **different** sequence of codes. Keep the settings identical on every system that needs to compute matching codes.
...
- **Nothing is sent anywhere.** The TOTP secret and every generated code stay in the browser's memory (and, optionally, in `localStorage`) and are never transmitted.
- **`localStorage` is not encrypted.** If you check "remember this secret on this device", the Base32 secret is stored in plain text in the browser's local storage for this page's origin. Anyone with access to that browser profile (or a malicious browser extension with broad permissions) could read it. Only use this option on a personal, trusted device, and use **Clear** to remove it when you're done.
...
**My code doesn't match my authenticator app for the same secret.**
...
Yes — paste the Base32 string associated with that secret into the TOTP secret field. This tool does not scan QR codes; you'll need the raw Base32 value.

**Does this store or transmit my secret anywhere outside my browser?**
